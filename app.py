...
if uploaded_file is not None:
    ...
    if model_loaded:
        ...
        if detected_class != "notumor":
            ...
            st.caption(
                f"A **{class_name}** was detected. Enter bounding box coordinates around "
                "the tumor region below, then click **Run MedSAM**."
            )

            img_rgb = cv2.cvtColor(img_array, cv2.COLOR_GRAY2RGB)
            h_orig, w_orig = img_rgb.shape[:2]

            st.image(img_rgb, caption=f"Scan — size: {w_orig} x {h_orig} px", use_container_width=True)
            st.info("Enter coordinates of the bounding box around the tumor, then click **Run MedSAM**.")
